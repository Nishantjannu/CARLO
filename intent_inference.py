    ...
    for _ in range(15):
        ...
        while not done:
            ...
            for goal_nr, goal in enumerate(goals[scenario_name]):
                ...
                try:
                    prob = multivariate_normal.pdf(action, mean=mean, cov=cov + np.eye(2, 2) * 1e-4)
                except:
                    prob = multivariate_normal.pdf(action, mean=mean, cov=cov + np.eye(2, 2) * 1e-1)
                probs[goal_nr] = prob
                denonminator += prob

            for prob_nr in range(len(probs)):
                probs[prob_nr] = probs[prob_nr] / denonminator
